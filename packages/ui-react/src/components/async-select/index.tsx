 ...
 **/
export const AsyncSelect = <IsMulti extends boolean = false>({
    isSearchable = false,
    isLoading = false,
    ...
    const elOptionSelected = useCallback((option: OptionProps<IOption, IsMulti, GroupBase<IOption>>) => {
        if(option.isSelected) {
            return (
                <IconCheckMark
                    svg={{
                        className: cn('async-select__icon-check-mark', {
                            'async-select__icon-check-mark_disabled': option.isDisabled
                        })
                    }}
                />
            );
    ...
    const componentIndicatorsContainer = useCallback((option: IndicatorsContainerProps<IOption, IsMulti, GroupBase<IOption>>) => (
        <components.IndicatorsContainer {...option}>
            <IconArrowsChevronBottom
                svg={{
                    className: cn('async-select__icon-arrow-bottom', {
                        'async-select__icon-arrow-bottom_disabled': option.isDisabled,
                        'async-select__icon-arrow-bottom_open': option.selectProps.menuIsOpen
                    })
                }}
            />
    ...
    const elLabel = useMemo(() => {
        if(props.label) {
            return (
                <span
                    className={cn('async-select__label')}
    ...
    const loadOptions = (
        search: string,
        prevOptions: OptionsOrGroups<IOptionType, GroupBase<IOptionType>>
    ...
    return (
        <label className={cn('async-select')}>
            {elLabel}
            <AsyncPaginate
                ...
                classNames={{
                    menu: () => cn('async-select__menu'),
                    menuList: () => cn('async-select__menu-list'),
                    singleValue: () => cn('async-select__single-value'),
                    control: (option) => cn('async-select__control', {
                        'async-select__control_disabled': option.isDisabled,
                        'async-select__control_error': props.isError
                    }),
                    valueContainer: () => cn('async-select__value-container'),
                    placeholder: () => cn('async-select__placeholder'),
                    option: (option) => cn('async-select__option', {
                        'async-select__option_focused': option.isFocused,
                        'async-select__option_disabled': option.isDisabled,
                        'async-select__option_selected': option.isSelected
                    })
                }}
                components={{
