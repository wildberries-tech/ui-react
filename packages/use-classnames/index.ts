...
import { useConstant } from '@wb-ui-react/use-constant';
...
interface TBinding<TLocals> {
    locals: Record<keyof TLocals, string>,
    use(): void,
    unuse(): void
}
...
export const useClassnamesLazy = <TTarget extends TBinding<TTarget['locals']>, TSource = Binding>(targetStyle: TTarget, sourceStyle?: Record<keyof TSource, string> | string, combine?: boolean) => {
    useConstant(targetStyle.use);

    return useMemo(() => classNames<TTarget['locals'], TSource>(targetStyle.locals, sourceStyle, combine), [targetStyle, sourceStyle, combine]);
};
